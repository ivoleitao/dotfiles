...
format_now_playing() {
    local value="$1"
    local icon="$NOW_PLAYING_ICON"

	if [ -n "$value" ]; then
		case "$NOW_PLAYING_METHOD" in
			"roll")
				value=$(panel_roll_text "${value}" ${NOW_PLAYING_MAX_LEN} ${NOW_PLAYING_ROLL_SPEED})
				;;
			"trim")
				value=${value:0:NOW_PLAYING_MAX_LEN}
				;;
		esac
	fi

    echo "<span foreground='$NOW_PLAYING_ICON_COLOR'>$icon</span> $value"
}

panel_now_playing() {
    local value="Stopped"
    local result="$(mpc current -h "$NOW_PLAYING_MPD_HOST" -p "$NOW_PLAYING_MPD_PORT" -f "$NOW_PLAYING_MPC_FORMAT" 2>&1)"
    if [ $? -eq 0 ] && [ -n "$result" ]; then
        value="$result"
    fi

    echo "$(format_now_playing "$value")"
}

echo $(panel_now_playing)
